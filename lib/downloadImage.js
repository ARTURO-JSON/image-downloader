 ...
 * 
 * Flow:
 * 1. Create a link to the server proxy endpoint with image details
 * 2. Trigger browser download which respects Content-Disposition header
 * 3. Server fetches the image and returns with proper file headers
 * 4. Browser saves file with correct name and extension
 * 
 * Fallback:
...
export async function downloadImage(fullImageUrl, imageId, source = 'unsplash') {
    try {
        ...
        // Proxy handles: URL validation, CORS, file streaming, error handling
