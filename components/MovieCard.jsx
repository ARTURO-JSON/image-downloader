...
export default function MovieCard({ movie, onClick }) {
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-xl bg-white shadow-md cursor-pointer transform transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-primary-500/20"
    ...
    >
      ...
      <div className="p-3 md:hidden bg-white">
        <h3 className="font-semibold text-gray-900 text-sm leading-tight line-clamp-1">
          {movie.title}
        </h3>
        <div className="flex items-center justify-between mt-1">
          <span className="text-gray-500 text-xs">{releaseYear}</span>
          <div className="flex items-center gap-1">
            <svg className="w-3 h-3 text-yellow-400 fill-current" viewBox="0 0 20 20">
              <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z" />
            </svg>
            <span className="text-gray-600 text-xs font-medium">{rating}</span>
          </div>
        </div>
      </div>
    </div>
